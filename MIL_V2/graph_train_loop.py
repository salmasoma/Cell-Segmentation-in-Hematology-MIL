...
def filter_invalid_edges_and_nodes(data):
    num_nodes = data.x.size(0)  # Number of nodes based on the node features tensor
    ...
    valid_node_mask = (data.x.dim() == 2)  # Check if node features are 2-dimensional
    if not valid_node_mask:
        data.invalid = True
    else:
        data.invalid = False
...
def train_graph_multi_wsi(graph_net, train_loader, test_loader, loss_fn, optimizer, n_classes, num_epochs=1, checkpoint=True, checkpoint_path="PATH_checkpoints"):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (patient_ID, graph_object) in enumerate(tqdm(train_loader.dataset.items(), desc='Training Progress')):
            ...
            data = filter_invalid_edges_and_nodes(data)
            ...
            if data.x.dim() != 2:
                print(patient_ID)
                raise ValueError(f"Expected data.x to be 2-dimensional, but got shape {data.x.shape}")
        ...
        # Calculate the most common prediction for each patient
        final_predictions = []
        for patient_ID, preds in patient_predictions.items():
            final_pred = max(set(preds), key=preds.count)
            final_predictions.append((patient_ID, final_pred))
        ...
        print()
        print('Epoch: {}, train_loss: {:.4f}, train_accuracy: {:.4f}, train_f1: {:.4f}'.format(epoch+1, total_loss, train_accuracy, train_f1))
        ...
        for batch_idx, (patient_ID, graph_object) in enumerate(tqdm(test_loader.dataset.items(), desc='Testing Progress')):
            ...
            patient_ID = patient_ID.split('_')[0]
            if patient_ID not in test_patient_predictions:
                test_patient_predictions[patient_ID] = []
            test_patient_predictions[patient_ID].append(Y_hat.item())
        ...
        for patient_ID, preds in test_patient_predictions.items():
            final_pred = max(set(preds), key=preds.count)
            final_test_predictions.append((patient_ID, final_pred))
...
def train_graph_multi_wsi(graph_net, train_loader, test_loader, loss_fn, optimizer, n_classes, num_epochs=1, checkpoint=True, checkpoint_path="PATH_checkpoints"):
    ...
    for epoch in range(num_epochs):
        ...
        for patient_ID, preds in patient_predictions.items():
            ...
            final_pred = subtype_index[list_count.index(max_count)]
