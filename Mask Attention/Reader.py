...
################################Initiate folders were files are and list of train images############################################################################
    def __init__(self, ImageDir,AnnotationDir, MaxBatchSize=100,MinSize=160,MaxSize=1100,MaxPixels=800*800*5, AnnotationFileType="png", ImageFileType="png",BackgroundClass=0, NumClasses=-1):
...
#-------------------Get All files in folder--------------------------------------------------------------------------------------
        ...
        # def scan_directory(current_dir, annotation_file_type="png", file_list=None):
        ...
        #             scan_directory(file_path, annotation_file_type, file_list)
        #         elif annotation_file_type in file_name:
        #             # If it is a file and contains the required file type, add to the list
        #             file_list.append(file_path)

        #     return file_list
            
        # self.FileList= scan_directory(AnnotationDir)
...
#-----------------------Get List o files for each class--------------------------------------------------------------------------
        ...
        print("Annotation Directory:", self.AnnotationDir)
        print("Files found:", len(self.FileList))
...
#=====================Initiate batch=============================================================================================
        ...
        BatchSize=np.int32(np.min((np.floor(self.MaxPixels/(Hb*Wb)),self.MaxBatchSize))) # Number of images in batch
        BImgs=np.zeros((BatchSize,Hb,Wb,3)) # Images
        BSegmentMask=np.zeros((BatchSize,Hb,Wb)) # Segment mask
        BLabels=np.zeros((BatchSize),dtype=np.int32) # Class
...
# Given an image number and a class the batch conssit on all the instances of the input class in the input image######################################################################################################
    def ReadNextImageClean(self,MaxBatchSize=40,MaxPixels=1500000):
        ...
        BLabels = np.zeros((MaxBatchSize), dtype=np.int32)  # Class
